...
class TemporalBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer_norm(x)
        x = x.permute(0, 2, 1)      # back to (N, C, T)
...
class TCNEncoder(nn.Module):
    """
    Wraps LocusLab TemporalConvNet to match baseline shape:
    Input:  (T, N, num_features)
    Output: (T, N, out_features)
    ...
    def __init__(
        ...
        self.output_dim = channel_sizes[-1]
